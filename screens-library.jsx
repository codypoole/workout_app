...
const EX_TYPES = [['weight','Weight × reps'],['bodyweight','Bodyweight reps'],['timed','Timed hold']];

function LibraryScreen({ state, onSave, onDelete, onOpenExercise }) {
  const [q, setQ] = useState('');
  const [group, setGroup] = useState('All');
  const [edit, setEdit] = useState(null); // exercise being edited or {new}
    ...
    <div className="screen screen-enter">
      ...
      <div style={{ padding:'8px 16px 8px' }}>
        <div className="row between" style={{marginBottom:12}}>
          <div><div className="eyebrow">Master library</div><div className="h1">Exercises</div></div>
          <button className="icon-btn" style={{width:44,height:44,background:'var(--accent)',color:'var(--accent-ink)',border:'none'}}
            onClick={()=>setEdit({ id:null, name:'', group:'Chest', equipment:'Barbell', type:'weight', muscles:[], notes:'', custom:true })}>
        ...
        <div className="row gap8" style={{ overflowX:'auto', paddingBottom:2 }}>
          {GROUPS.map(g=>(
            <button key={g} className="chip" style={{height:32,padding:'0 13px',
              ...(g===group?{background:'var(--accent)',color:'var(--accent-ink)',borderColor:'transparent'}:{})}}
          ...
          {Object.entries(byGroup).map(([g, items]) => (
            <div key={g} style={{ marginBottom:18 }}>
              <div className="row gap8" style={{ marginBottom:8 }}>
                <GroupDot group={g} size={8}/><span className="eyebrow">{g}</span>
                ...
                {items.map(ex => (
                  <div key={ex.id} className="card" style={{ padding:12 }}>
                    <div className="row between">
                      <button style={{ background:'none',border:'none',textAlign:'left',cursor:'pointer',flex:1, minWidth:0 }}
                      ...
                      <button className="icon-btn" style={{width:36,height:36}} onClick={()=>setEdit({...ex})}><Icon name="edit" size={16}/></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <Sheet open={!!edit} onClose={()=>setEdit(null)} title={edit && edit.id ? 'Edit exercise' : 'New exercise'} full>
        {edit && <ExerciseEditor key={edit.id||'new'} ex={edit}
...
function ExerciseEditor({ ex, onSave, onDelete }) {
  ...
  function save() {
    ...
    onSave({
      id: ex.id || DB.uid('ex'),
      name: name.trim(), group, equipment, type,
      muscles: muscles.split(',').map(m=>m.trim()).filter(Boolean),
      notes, custom: ex.id ? ex.custom : true,
    });
  }
    ...
    <div className="col gap16" style={{ paddingTop:6 }}>
      ...
      <div className="col gap6">
        ...
        <div className="row gap6 wrap">
          {GROUPS.slice(1).map(g=>(
            <button key={g} className="chip" style={{height:34,padding:'0 13px',
              ...(g===group?{background:'var(--accent)',color:'var(--accent-ink)',borderColor:'transparent'}:{})}}
              onClick={()=>setGroup(g)}>{g}</button>
          ))}
        </div>
      ...
      <div className="col gap6">
        ...
        <div className="row gap6 wrap">
          {EQUIPMENT.map(g=>(
            <button key={g} className="chip" style={{height:34,padding:'0 13px',
              ...(g===equipment?{background:'var(--surface-3)',color:'var(--text)',borderColor:'var(--line-2)'}:{})}}
              onClick={()=>setEquipment(g)}>{g}</button>
          ))}
        </div>
      </div>
      <div className="col gap6">
        ...
        <div className="col gap6">
          {EX_TYPES.map(([val,lbl])=>(
            <button key={val} className="card-2" style={{ padding:'12px 14px', textAlign:'left', cursor:'pointer',
              borderColor: type===val?'var(--accent)':'var(--line)' }} onClick={()=>setType(val)}>
              <div className="row between">
                <span className="title" style={{fontSize:14}}>{lbl}</span>
                {type===val && <span className="accent"><Icon name="check" size={16}/></span>}
              </div>
            </button>
          ))}
        </div>
      ...
      <button className="btn primary block lg" onClick={save}><Icon name="check" size={18}/> Save exercise</button>
      {onDelete && <button className="btn danger block" onClick={onDelete}><Icon name="trash" size={16}/> Delete exercise</button>}
    </div>
  );
}

/* ---- Swap sheet (pick a replacement from library) ---- */
function SwapSheet({ open, onClose, library, currentId, onPick, mode }) {
  const [q, setQ] = useState('');
  ...
  // sort: same group first (swap mode)
  if (!isAdd) list.sort((a,b)=> (a.group===((cur||{}).group)?-1:0) - (b.group===((cur||{}).group)?-1:0));
  return (
    <Sheet open={open} onClose={onClose} title={isAdd ? 'Add exercise' : 'Swap exercise'} full>
      {isAdd
        ...
        : cur && <div className="label" style={{marginBottom:10}}>Replacing <span className="accent">{cur.name}</span>. Sets & reps are kept.</div>}
      ...
      <div className="col gap8">
        {list.map(ex=>(
          <button key={ex.id} className="card" style={{ padding:12, textAlign:'left', cursor:'pointer' }}
            ...
            <div className="row between">
              ...
              {!isAdd && cur && ex.group===cur.group && <span className="chip accent" style={{padding:'2px 7px', flexShrink:0}}>Similar</span>}
              {isAdd && <span className="accent" style={{flexShrink:0}}><Icon name="plus" size={18}/></span>}
            </div>
          </button>
        ))}
      </div>
...
Object.assign(window, { LibraryScreen, SwapSheet });
